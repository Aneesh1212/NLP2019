...
def calculate_transition_probs(labels):
    ...
    for label_set in labels:
        for i in range(len(label_set)):
            ...
            counts[prev_label + curr_label] = counts.get(prev_label + curr_label, 0) + 1
    return counts
    ...
    for x in range(len(labels)):
        for i in range(len(labels[x])):
            ...
            if(not curr_word+curr_label in counts.keys()):
              counts["<UNK>"] +=1
            counts[curr_word + curr_label] = counts.get(curr_word + curr_label, 0) + 1
    return counts
...
def viterbi_segment(text, pos_seq, counts, classifier, hmmcounts, isHMM):
    ...
    SCORE[0][0] = (counts["<s>0"]/counts["<s>"]) * classifier.prob_classify(featureset=feature_dict).prob(0)
    SCORE[1][0] = (counts["<s>1"]/counts["<s>"]) * classifier.prob_classify(featureset=feature_dict).prob(1)
    ...
    for t in range(1, n):
        if(not isHMM):
          ...
          if SCORE[0][t-1]*(counts["00"]/counts["0"]) >= SCORE[1][t-1] * (counts["10"]/counts["1"]):
              ...
              SCORE[0][t] = SCORE[0][t-1] * classifier.prob_classify(feature_dict).prob(0)
              BPTR[0][t] = 0
          else:
              #SCORE[0][t] = SCORE[1][t-1]* (counts["10"]/counts["1"]) * classifier.prob_classify(feature_dict).prob(0)
              SCORE[0][t] = SCORE[1][t-1] * classifier.prob_classify(feature_dict).prob(0)
              BPTR[0][t] = 1
          ...
          if SCORE[0][t-1]*(counts["01"]/counts["0"]) >= SCORE[1][t-1] * (counts["11"]+1000/counts["1"]):
              ...
              SCORE[1][t] = SCORE[0][t-1] * (classifier.prob_classify(feature_dict).prob(1)+.5 )
              BPTR[1][t] = 0
          else:
              #SCORE[1][t] = SCORE[1][t-1]*(counts["11"]/counts["1"]) * classifier.prob_classify(feature_dict).prob(1)
              SCORE[1][t] = SCORE[1][t-1] * (classifier.prob_classify(feature_dict).prob(1)+.5 )
              BPTR[1][t] = 1
        else:
          word0 = "<UNK>"
          word1 = "<UNK>"
          ...
          if SCORE[0][t-1]*(counts["00"]/counts["0"]) >= SCORE[1][t-1] * (counts["10"]/counts["1"]):
              SCORE[0][t] = SCORE[0][t-1]* (counts["00"]/counts["0"]) * (hmmcounts[word0]/counts["0"])
              BPTR[0][t] = 0
          else:
              SCORE[0][t] = SCORE[1][t-1]* (counts["10"]/counts["1"]) * (hmmcounts[word0]/counts["0"])
              BPTR[0][t] = 1

          if SCORE[0][t-1]*(counts["01"]/counts["0"]) >= SCORE[1][t-1] * (counts["11"]/counts["1"]):
              SCORE[1][t] = SCORE[0][t-1]*(counts["01"]/counts["0"]) * (hmmcounts[word1]/counts["1"])
              BPTR[1][t] = 0
          else:
              SCORE[1][t] = SCORE[1][t-1]*(counts["11"]/counts["1"]) * (hmmcounts[word1]/counts["1"])
              BPTR[1][t] = 1
